...
public:
    ...
    int* allocate(int n) {
        ...
        Allocation rec{ptr, static_cast<int>(blocks_.size() - 1), start, n, false};
    ...
    void deallocate(int* pointer, int /*n*/) {
        ...
        if (rec.freed) return; // already freed
        rec.freed = true;
        ...
        while (!allocs_.empty() && allocs_.back().freed) {
            ...
            // rollback bump pointer for its block
            ...
            // If the last block becomes empty and it's the tail block, release it
            while (!blocks_.empty()) {
